...
-- ── STORES ───────────────────────────────────────────────
-- CREATE OR REPLACE resets the AUTOINCREMENT sequence to 1..N on every run so
-- that child foreign keys populated with UNIFORM(1,N) always resolve.
...
-- ── CUSTOMERS (reload with 500 rows via GENERATOR) ──────
